...
class addentry {
    ...
    function clear_tmpfiles($cachetime=1800) {
        global $GB_TMP;
        $delfiles = 0;
		$tmpDir = $this->include_path.'/'.$GB_TMP;
		if (is_dir($tmpDir)) {
			$fh = opendir($tmpDir);
			if ($fh !== false) {
				$now = time();
				while (($file = readdir($fh)) !== false) {
					...
					$matches = array();
					if (preg_match("/img-([0-9]+)\.[a-z0-9]+$/i", $file, $matches)) {
						if (isset($matches[1])) {
							$filetime = intval($matches[1]);
							if ($filetime < $now-$cachetime) {
								if (unlink($tmpDir.'/'.$file)) {
									$delfiles++;
								}
							}
						}
    ...
    function check_entry($type='') {
        ...
        if ($this->name == "") {
            ...
            if (!preg_match('/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@([0-9a-z][0-9a-z-]*[0-9a-z]\.)+[a-z]{2,5}$/', $this->email)) {
                    //XXX: very simple heuristic, still much work to do
                $this->email = '';
            }
            if (!preg_match('/^https?:\/\/[a-z0-9-]+\.[a-z0-9-]+/', $this->url)) {
                    //XXX: very simple heuristic, still much work to do
                $this->url = '';
            }
        ...
        if ($this->canUseCaptcha) {
        	$cs = new CaptchaService(DataSourceSessionImpl::getInstance());
        	if (get_magic_quotes_gpc()) {
        		$this->gb_captcha = stripslashes($this->gb_captcha);
        	}
			if (!$cs->isValidResponse($this->token, $this->gb_captcha)) {
				$cs->removeChallenge($this->token);
				return $this->db->gb_error($this->db->LANG["ErrorPost4"]);
			}
    	}
    ...
    function form_addguest() {
        ...
        if ($this->canUseCaptcha) {
			...
			$_locale = guestbook_vars::getLocale($VARS['lang']);
        	$dssi->set('AddRequestIp', $_SERVER['REMOTE_ADDR']);
    ...
    function preview_entry() {
        ...
        if ($this->db->VARS["show_ip"] == 1) {
            $hostname = ( !preg_match('/^([0-9]{1,3}\.){3}[0-9]{1,3}$/', $host) ) ? "Host" : "IP";
            $HOST = "$hostname: $host\n";
        } else {
            $HOST = '';
        }
