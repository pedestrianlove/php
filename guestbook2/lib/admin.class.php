...
class gb_admin {
    ...
    function scan_smilie_dir() {
        global $include_path;
        $smiley_path = $include_path . '/img/smilies';

        $smilie_list = array();
        $hnd = @opendir($smiley_path);
        while (($file = readdir($hnd)) !== FALSE) {
            if (is_file($smiley_path . '/' . $file) && 
                    preg_match('/\.(gif|jpg|png|jpeg)$/', "$file")) {
                $smilie_list[] = $file;
            }
        }
        closedir($hnd);
        ...
        $smilies = array();
        if (!empty($smilie_list)) {
            $smilies = array();
            asort($smilie_list);
            foreach ($smilie_list as $smiley) {
                $size = getimagesize($smiley_path . '/' . $smiley);
                if (is_array($size)) {
                    $smilies[$smiley] = "<img src=\"img/smilies/$smiley\" $size[3]>";
                }
            }
        }
        return $smilies;
    ...
    function update_record($entry_id,$tbl="gb") {
        ...
        if (!preg_match('/.+@[-a-z0-9\.]+$/', $_POST['email'])) {
                    // FIXME: does not match IDNs properly.
            $_POST['email'] = '';
        }
